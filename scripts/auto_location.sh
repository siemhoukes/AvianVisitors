...
# Distance from the configured location. A fresh install sits at 0,0 -> adopt
# the first reading immediately. Otherwise only relocate on a real hop so a
# same-site reboot doesn't churn services over a few km of IP wobble.
...
# Apply: rewrite LATITUDE/LONGITUDE via a redirect (not sed -i) so the file
# keeps its owner/permissions - the web UI (caddy user) writes it directly too.
...
log "location set to ${place:-unknown}: ($oldlat,$oldlon) -> ($clat,$clon), moved ${moved} km - reloading detection"
# Services started moments ago at boot with the OLD location; restart so the
# analyzer reloads the species list for the new spot. (Once per actual move.)
